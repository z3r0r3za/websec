...
def exploit_sqli_creds(sess, num_col, url, path_param):
    # Convert URL to the correct one and get the token before attempting to log in.
...
if __name__ == "__main__":
    try:
        ...
        print(
            '[-] Example: %s orderby https://website.net "/filter?category=Gifts"'
            % sys.argv[0]
        )
        print(
            '[-] Example: %s union https://website.net "/filter?category=Gifts"'
            % sys.argv[0]
        )
        sys.exit(-1)
